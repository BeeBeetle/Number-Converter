...
namespace Converter
{
    public partial class Converter : Form
    {
        ...
        private void btnConvert_Click(object sender, EventArgs e)
        {
            ...
            //if the two bases match (a silly case but w/e)
            if (cmbOutput.SelectedIndex == cmbInput.SelectedIndex)
            {
                rtbOutput.Text = rtbInput.Text;
            }
            //converting from base 10 to any other base (easy)
            else if (cmbInput.SelectedIndex == 3)
            {
                rtbOutput.Text = numberChanger.ConvertFrom10(Convert.ToUInt64(rtbInput.Text), Convert.ToUInt64(cmbOutput.SelectedValue));
            }
            //if converting from not base 10 to any other base
            //it is important to convert to base 10 first because the math to convert to an alternate base
            //requires that you be in base 10 because there is no math you can do outside of base 10
            else if (cmbInput.SelectedIndex != 3)
            {
                string base10 = numberChanger.ConvertTo10(rtbInput.Text, Convert.ToUInt64(cmbInput.SelectedValue));
                rtbOutput.Text = numberChanger.ConvertFrom10(Convert.ToUInt64(base10), Convert.ToUInt64(cmbOutput.SelectedValue));
            }
        }
    }
}